...
public class QuiltRegistryHandler<T> implements RegistryHandler<T> {
	...
	private QuiltRegistryHandler(ResourceKey<Registry<T>> registryKey, String namespace) {
		this.registryKey = registryKey;
		this.namespace = namespace;
		this.registry = Suppliers.memoize(() -> RegistryHelper.getBuiltinRegistry(registryKey));
	}

	public static <T> QuiltRegistryHandler<T> create(ResourceKey<Registry<T>> registryKey, String modid) {
		return new QuiltRegistryHandler<>(registryKey, modid);
	}
	...
	@Override
	public <S extends T> QuiltRegistrySupplier<T, S> register(ResourceKey<T> id, Supplier<S> factory) {
		...
		var supplier = new QuiltRegistrySupplier<T, S>(id, factory);
